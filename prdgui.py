...
Further improvements from Dango233 and nsheppard helped improve# -*- coding: utf-8 -*-
...
def stop_thread():
    ...
    if is_running == True:
        ...
        print("Not Running", file = sys.stdout)
...
def refresh():
    ...
    try:
        if json_set['text_prompts']['0']['0']:
            ...
            prompt_text = json_set['text_prompts']['0']['0']
    except:
    ...
    if prompt_scheduling == True:
        for i in prompt_steps:
            for k in prompt_text.keys():
                prompt_text_vars[i][k].set(json_set['text_prompts'][i][k])
                prompt_text_entry[i].configure(textvariable=prompt_text_vars[i][k])
    else:
        for k in prompt_text:
            for i in range(len(prompt_text_entry)):
                print(k+' test')
                prompt_text_vars[i].set(k)
                prompt_text_entry[i].configure(textvariable=prompt_text_vars[i])
...
def updater():
    ...
    if prompt_scheduling == True:
        text_prompts_button_frame.pack()
        for i in prompt_steps:
            for k in prompt_text_entry[i].keys():
                prompt_text_entry_text = prompt_text_entry[i][k].get()
                json_set['text_prompts'][i] = prompt_text_entry_text
    else:
        temp_prompts = []
        t = 0
        text_prompts_button_frame.pack_forget()
        for i in range(len(prompt_text_entry)):
            prompt_text_entry_text = prompt_text_entry[i].get()
...
# Middle Bottom Frame
text_prompts_button_frame = ttk.Frame(left_frame_bottom, style='Grey.TFrame')
text_prompts_button_frame.pack(side=TOP, anchor=NW, fill=BOTH, padx=2, pady=2)
...
# Stop Button
stop_button = ttk.Button(basic_settings_frame, text="Stop", command=stop_thread)
stop_button.grid(row=1, column=6, sticky=NW, padx=5, pady=2)
...
def get_prompts():
    ...
    if prompt_scheduling == True:
        ...
        for k in prompt_steps:
            ...
            prompt_text_entry[k] = {}
            prompt_text_vars[k] = {}
            prompt_step_button = ttk.Button(text_prompts_button_frame, text="Step " + k, command=lambda k=k: show_prompt_step(k))
            prompt_step_button.pack(side=LEFT, padx=5, pady=2)
...
sys.stdout = old_stdout the quality of diffusion in general, and especially so for shorter runs like this notebook aims to achieve.
Vark added code to load in multiple Clip models at once, which all prompts are evaluated against, which may greatly improve accuracy.
The latest zoom, pan, rotation, and keyframes features were taken from Chigozie Nri's VQGAN Zoom Notebook (https://github.com/chigozienri, https://twitter.com/chigozienri)
Advanced DangoCutn Cutout method is also from Dango223.
Somnai (https://twitter.com/Somnai_dreams) added Diffusion Animation techniques, QoL improvements and various implementations of tech and techniques, mostly listed in the changelog below.
